import * as ROT from "rot-js";
import { Guard, Adventurer, Barmaid, Villager, ActorState, Cat } from "./Actor";
import { Item } from "./Item";
import { Game } from "./Game";

export function setupWorld(game: Game): void {
  const { state, scheduler } = game;

  scheduler.add(state.player, true);

  let eggLocation = Math.floor(ROT.RNG.getUniform() * 3);
  setupVillagers(game, eggLocation);
  placeEgg(game, eggLocation);
  placeCloak(game);

  scheduler.add({
    act: () => {
      state.turn++;
      state.checkAngerSubsides();
      return Promise.resolve();
    }
  }, true);
}

function placeEgg(game: Game, eggLocation: number): void {
  const { state } = game;
  let x: number, y: number;

  switch (eggLocation) {
    case 0: // alchemist
      x = 134;
      y = 32;
      break;
    case 1: // mayor
      x = 180;
      y = 25;
      break;
    default: // temple
      x = 161;
      y = 6;
      break;
  }

  const egg = new Item(x, y, "dragon egg", "A leathery egg emanating warmth and a faint glow.", "O", "#f2f0e5");
  state.items[`${x},${y}`] = egg;
}

function placeCloak(game: Game): void {
  const { state } = game;
  const options: [number, number][] = [[91, 7], [117, 14], [135, 4], [149, 21]];
  const [x, y] = options[Math.floor(ROT.RNG.getUniform() * 4)];

  const cloak = new Item(x, y, "tattered cloak", "While wearing this, the dumber humans might not recognize you.", "(", "#a77b5b");
  state.items[`${x},${y}`] = cloak;
}

function setupVillagers(game: Game, eggLocation: number): void {
  const { state, scheduler } = game;

  const patrolPath: [number, number][] = [
    [107, 10], [135, 10], [135, 11], [140, 11], [140, 12], [141, 12],
    [141, 24], [132, 24], [132, 25], [131, 25], [131, 26], [123, 26],
    [123, 23], [115, 23], [115, 20], [113, 20], [113, 19], [107, 19],
  ];

  const guard1 = new Guard(107, 10, "#b8b5b9", "Guard", ActorState.Patrolling, state, patrolPath);
  guard1.description = "A stinky human who's carrying a sword.";
  scheduler.add(guard1, true);
  state.villagers.push(guard1);

  const guard2 = new Guard(160, 12, "#b8b5b9", "Guard", ActorState.Guarding, state);
  guard2.description = "A stinky human who's holding a spear.";
  guard2.facingDx = 0;
  guard2.facingDy = 1;
  scheduler.add(guard2, true);
  state.villagers.push(guard2);

  const guard3 = new Guard(168, 20, "#b8b5b9", "Guard", ActorState.Guarding, state);
  guard3.description = "A stinky human who's holding a club.";
  guard3.facingDx = 0;
  guard3.facingDy = -1;
  scheduler.add(guard3, true);
  state.villagers.push(guard3);

  const guard4 = new Guard(182, 26, "#b8b5b9", "Guard", ActorState.Guarding, state);
  guard4.description = "A stinky human with a guisarme.";
  guard4.facingDx = -1;
  guard4.facingDy = -1;
  scheduler.add(guard4, true);
  state.villagers.push(guard4);

  let b1 = "";
  let b2 = "";
  let b3 = "";
  switch (eggLocation) {
    case 0:
      b1 = "The alchemist will make good use of the egg!";
      b2 = "The alchemist had a lot of shiny things in his shop...";
      b3 = "Can't believe we sold the egg to that dork!";
      break;
    case 1:
      b1 = "Is the mayor really going to eat the dragon egg?";
      b2 = "The mayor has a guard protecting the egg.";
      b3 = "They could have at least invited us to the feast.";
      break;
    case 2:
      b1 = "What does a priest want with a dragon egg?";
      b2 = "The priest is so judgmental.";
      b3 = "I'm glad a holy man has the egg now.";
      break;
  }

  const adven1 = new Adventurer(97, 23, "#cf8acb", "Sorceress", [
    "My mana is low, but it was worth it.",
    "Another cup of wine, please.",
    "Hmm. Bloodstains on my robe.",
    b1
  ], state);
  adven1.description = "One of the stinky humans what robbed your clan.";
  scheduler.add(adven1, true);
  state.villagers.push(adven1);

  const adven2 = new Adventurer(98, 24, "#45444f", "Rogue", [
    "I wish the kobolds had more treasure!",
    "There were so many traps.",
    "You guys were too noisy.",
    b2
  ], state);
  adven2.description = "One of the stinky humans what robbed your clan.";
  scheduler.add(adven2, true);
  state.villagers.push(adven2);

  const adven3 = new Adventurer(96, 24, "#c2d368", "Warrior", [
    "God, that cave was stinky.",
    "Nothing like good ale after a dungeon crawl!",
    "Maybe I'll by a new sword.",
    b3
  ], state);
  adven3.description = "One of the stinky humans what robbed your clan.";
  scheduler.add(adven3, true);
  state.villagers.push(adven3);

  const barmaid = new Barmaid(96, 21, "#edc8c4", "Barmaid", state);
  barmaid.description = "A stinky human carrying a tray of mugs.";
  scheduler.add(barmaid, true);
  state.villagers.push(barmaid);

  const barfly1 = new Villager(95, 18, "#4b80ca", "Barfly", state);
  barfly1.description = "Another stinky human.";
  scheduler.add(barfly1, true);
  state.villagers.push(barfly1);

  const priest = new Villager(161, 6, "#6a536e", "Priest", state);
  priest.description = "This stinky human is wearing a fancy-looking necklace.";
  scheduler.add(priest, true);
  state.villagers.push(priest);

  const alchemist = new Villager(130, 32, "#cf8acb", "Alchemist", state);
  alchemist.description = "This stinky human smells like chemicals and duckweed.";
  scheduler.add(alchemist, true);
  state.villagers.push(alchemist);

  const familiar = new Cat(132, 34, state);
  scheduler.add(familiar, true);
  state.villagers.push(familiar);
}
